...
### Властивості об'єкта, їх конфігурації {#1-6}

Влас-ть даних має значення `value` та прапори: `writable` - можливість змінити значення,  
`enumerable` - видимий в циклах, `configurable` - може змінювати прапори.  
...
Вказаний параметр в об'єкті desctiptor буде змінений у властивості.  
...
Запис/видалення алвстивостей даних здійснюються безпосередньо на об'єкті.  
Методи про відсутності в об'єкта викликаються з прототипа.  
...
При зміні `F.prototype` `[[Prototype]]` попередніх вже створених об-в не змінюється.  
