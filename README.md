...
Оператор `typeof` повертає назву типу. (typeof 0 // "number")
...
`-0 == +0`, `-0 === +0`, але `Object.is(-0, +0) === false`;  
`NaN != NaN`, `NaN !== NaN`, але `Object.is(NaN, NaN) === true`.

Умовний оператор: `if(bool)`, `else`; `bool ? true : false`

Логічні оператори: `&&`, `||`, `!`, `??` (об’єднання з null `a ?? b` => `(a !== null && a!== undefined) ? a : b`)  
`switch(x) {case val1: break}` якщо x === val1
Оператор `||` повертає перше правдиве значення (result = val1 || val2)

Оператори циклу: `while(b){}`,`do{}while(b)`, `for(v;b;m){}`, `continue`, `break`, `break labelName` (зупинить `labelName: for(){}`)

Функції: `function funcName(args[]){}`, `funcName(args[]);` (створюються до виконання алгоритму, може виконуватись будь-де)  
Функ. вираз: `let funcNm = function(){};` (створюються підчас виконання алгоритму, тільки після оголошення)  
Функ.-колбек (передається як аргумент та виконується у відповідь на роботу основної ф-ї): `function funcNm(func){}`, `funcNm(anotherFunc);`  
...
`for (let key in obj) {}` - цикл перебору ключів контейнера (крім символьних).  
...
`new Array(n)`, `new Array(n)` створюють масив з n порожніми елементами.  
...
`arr.length` - властивість зі значенням на 1 більшим за останній індекс. Її зміна дає можливість вкоротити масив.  
...
`arr.flat(deep=1)` повертає масив з елементами підмасиву рекурсивно до глибини.  
...
`arr.every(func())` повертає true якщо всі виклики повернули true.  
`arr.some(func())` повертає true якщо хоча б один виклик повернув true.  
...
`getFullYear()`, `getMonth()`, `getDate()`, `getHours()`, `getMinutes()`, `getSeconds()`, `getMilliseconds()`, `getDay()` повертають відповідний параметр.  
...
`setFullYear(year, month=, date=)`, `setMonth(month, date=)`, `setDate(date)`, `setHours(hour, min=, sec=, ms=)`, `setMinutes(min, sec=, ms=)`, `setSeconds(sec, ms=)`, `setMilliseconds(ms)` та їх UTC-аналоги встановлюють відповідні параметр(и).  
...
`Date.parse(str)` читає дату із рядка виду `YYYY-MM-DDTHH:mm:ss.sssZ`, `YYYY-MM-DD`, `YYYY-MM`, `YYYY`. Неправильна строка - NaN.  
...
`JSON.stringify(val, replr=, spc=)` повертає JSON-кодований (серіалізований) об'єкт. `replr` масив власт-й для кодування або ф-я відобр-я. `spc` к-ть пробілів форматування.  
...
**Залишкові параметри** `...` дозволяють передати довільну к-ть аргументів як масив `function func(a, b, ...rest){}`. Мають бути в кінці, звертаємся за ім'ям (rest).  
`arguments` - ітерабельний псевдомасив всіх аргументів будь-якох ф-ї, крім стрілкових.  
**Синтаксис розширення** `...` розпаковує _ітеративний об'єкт_ до списку аргументів `func(a, b, ...arr);` передасть список аргументів взятих зі значень arr.  
...
Кожен блок, ф-я та скрипт вцілому мають власний асоціативний об-т **Лексичне середовище**, яке складається з _запису середовища_ (зберігає локальні змінні як властивості та ін) та посилання на _зовнішнє лексичне середовище_ (або null).  
...
`.cssText` повністю заміняє значення HTML-атрибута style новим рядком.  
